...
public class ProductDaoTest {
    ...
    public void init() {
        ...
        doAnswer(answer -> {
            ...
            return null;
        }).when(database).update(startsWith(INSERT_SQL_PREFIX));

        doAnswer(answer -> products.size()).when(database).query(eq(FIND_COUNT_SQL), any());

        doAnswer(answer -> products.stream().mapToLong(Product::getPrice).sum())
                .when(database)
                .query(eq(FIND_SUM_PRICE_SQL), any());
        ...
        }).when(database).query(startsWith(FIND_SQL_PREFIX), any());
